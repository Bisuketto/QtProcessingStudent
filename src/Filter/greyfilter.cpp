...
GreyFilter::GreyFilter(QString _name, unsigned int _color_values) : Filter(_name)
{
...
void GreyFilter::process(FastImage *_buffIn, FastImage *_buffOut){
    ...
    int R_coef = ((color_values & 0x00FF0000) >> 16); // Static point
    int G_coef = ((color_values & 0x0000FF00) >> 8 );
    int B_coef = ((color_values & 0x000000FF) >> 0 );
    int grey_value = 0;
