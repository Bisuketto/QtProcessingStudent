    ...
    if(buffTemp[0]->width() != _buffIn->width() || buffTemp[0]->height() != _buffIn->height()){
        ...
        for(int y=0; y<_buffIn->height(); y++){
            for(int x=0; x<_buffIn->width(); x++){
                    _buffOut->Red  (y, x, (_buffIn->Red(y, x) >> 7) + ((77*buffTemp[0]->Red(y, x)) >> 8) + ((51*buffTemp[1]->Red(y, x)) >> 8));
                    _buffOut->Green(y, x, (_buffIn->Green(y, x) >> 7) + ((77*buffTemp[0]->Green(y, x)) >> 8) + ((51*buffTemp[1]->Green(y, x)) >> 8));
                    _buffOut->Blue (y, x, (_buffIn->Blue(y, x) >> 7) + ((77*buffTemp[0]->Blue(y, x)) >> 8) + ((51*buffTemp[1]->Blue(y, x)) >> 8));
            }
        }
